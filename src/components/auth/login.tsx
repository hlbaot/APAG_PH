...
// Sinh mã xác nhận ngẫu nhiên 4 ký tự tại client
...
export default function LoginForm() {
  ...
  // Xử lý đăng nhập tĩnh tại client
  const handleLogin = () => {
    setIsLoading(true);
    setErrorMessage(null);
    setSuccessMessage(null);

    // Giả lập xử lý đăng nhập 500ms
    setTimeout(() => {
      try {
        localStorage.setItem("accessToken", `static-token-${Date.now()}`);
        ...
        setErrorMessage("Có lỗi xảy ra khi lưu phiên đăng nhập.");
      } finally {
        setIsLoading(false);
      }
    }, 500);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (captchaInput.trim().toUpperCase() !== captchaCode.toUpperCase()) {
      setErrorMessage("Mã xác nhận không chính xác. Vui lòng thử lại.");
      refreshCaptcha();
      return;
    }
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-3.5">
            ...
            <div className="flex gap-2 items-center">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <div
                  className="w-[110px] h-[38px] rounded border border-dashed border-blue-300 bg-blue-50/60 flex items-center justify-center overflow-hidden text-blue-700 font-mono text-lg italic font-bold tracking-widest select-none shadow-inner"
                  aria-label="Ảnh mã xác nhận"
                >
                  {captchaCode ? (
                    <span className="text-blue-700 font-mono text-lg italic font-bold tracking-widest">
                      {captchaCode}
                    </span>
                  ) : (
